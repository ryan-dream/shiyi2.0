...
Page({
    ...
    onPageScroll: function(t) {
        console.log(t.scrollTop), t.scrollTop > 300 ? this.setData({
            showTop: !1
        }) : this.setData({
            showTop: !0
        });
    ...
    getDatum: function(t) {
        ...
        n.request("entry/wxapp/Detail", {
            ...
            !1;
            t.article.content && 2 != t.article.type && i.wxParse("content", "html", t.article.content, e, 20), 
            t.article.explain && 2 != t.article.type && i.wxParse("explain", "html", t.article.explain, e, 20), 
            t.article.description && 2 != t.article.type && i.wxParse("description", "html", t.article.description, e, 20), 
            t.article.explaindesc && 2 != t.article.type && i.wxParse("explaindesc", "html", t.article.explaindesc, e, 20);
            var o = void 0;
            ...
            }), t.article.bg_music, a.onPlay) : t.article.bg_music != r.path ? (wx.onBackgroundAudioStop(a.onStop), 
            wx.setStorageSync(s, {
                id: t.article.id,
                path: t.article.bg_music
            }), t.article.bg_music, a.onPlay) : (wx.onBackgroundAudioStop(a.onStop), wx.getBackgroundAudioPlayerState({
                success: function(t) {
    ...
    createPosterImg: function(t, a) {
        ...
        wx.getImageInfo({
            ...
            success: function(r) {
                ...
                var f = u + 30, g = i.ccate1 ? "/" + i.ccate1 : "", m = i.author + " " + i.pcate1 + g;
                o.measureText(m).width >= .5 * n && (m = i.author, o.measureText(m).width >= .5 * n && (m = i.pcate1 + "/" + i.ccate1), 
                o.measureText(m).width >= .5 * n && (m = "")), o.fillText(m, d, f), o.fillText(i.createtime, .65 * n, f);
                var w, y = f + 15;
                o.setFontSize(14);
                var x = Math.floor(.73 * n), v = d, S = y, P = v + x, _ = S + Math.floor(4 * x / 16);
                if (_ > Math.floor(.4 * s) && (_ = Math.floor(.4 * s)), o.setFillStyle("#e9e9e9"), 
                o.fillRect(v + 4, S + 4, P + 6, _ + 4), "" != a.tmpPath ? o.drawImage(a.tmpPath, v, S, P, _) : (o.setFillStyle("#a6a6a6"), 
                o.fillRect(v, S, P, _)), 3 == i.types || 4 == i.types) {
                    o.setFillStyle("rgba(0,0,0,0.6)"), o.fillRect(v, S, P, _);
                    var T = v + (P - v) / 2 - 12, I = S + (_ - S) / 2 + 25;
                    o.drawImage("../../../resource/images/artposter/bofang.png", T, I, .14 * n, .14 * n);
                }
                w = y + _ + 45, o.drawImage("../../../resource/images/artposter/dou_1.png", v, w - 25, .05 * n, .03 * s), 
                o.drawImage("../../../resource/images/artposter/dou_2.png", P + 6, Math.floor(.78 * s) - 27, .05 * n, .03 * s), 
                o.setFontSize(12), o.setFillStyle("#333333");
                var D = i.description.replace(/<[^>]+>/g, ""), b = Math.floor(.86 * n) - Math.floor(.12 * n), M = D.split("");
                M[0] = "       " + M[0];
                for (var k = "", A = [], z = 0; z < M.length; z++) o.measureText(k).width < b ? k += M[z] : (z--, 
                A.push(k), k = "");
                if (A.push(k), 10 < A.length) {
                    for (var L = A.slice(0, 10), B = L[9], F = "", q = [], V = 0; V < B.length && o.measureText(F).width < b; V++) F += B[V];
                    q.push(F);
                    var C = q[0] + "...";
                    L.splice(9, 1, C), A = L;
                }
                if (w -= 10, 2 != i.types) {
                    if ("" != i.description && "undefined" != i.description && i.description.indexOf("<img") < 0 && i.description.indexOf("<video") < 0) {
                        for (var U = [], R = [], N = 0; N < A.length; N++) U[N] = w + 14 * (N + 1), U[N] = 0 == N ? U[N] : U[N] + 5 * N, 
                        R.push(A[N]), parseInt(U[N]) > parseInt(Math.floor(.78 * s) - 30) && R.pop();
                        R[R.length - 1] = R[R.length - 1] + "...", U = U.slice(0, [ R.length ]);
                        for (var H = 0; H < R.length; H++) H == R.length - 1 && (R[H] = R[H].substr(0, 13) + "..."), 
                        o.fillText(R[H], Math.floor(.1 * n), U[H]);
                    }
                ...
                o.drawImage(j, d, c, l, h), o.setFillStyle("#f2f2f2"), d = Math.floor(.65 * n), 
                c = Math.floor(.82 * s), l = Math.floor(.25 * n), h = .15 * s;
                var O = t.dg_article_title;
                o.setFontSize(16);
                var W = c + h / 3 + 10;
                o.fillText(O, Math.floor(.07 * n), W), o.draw(!0, setTimeout(function() {
                    e.cPosImg();
                }, 1e3));
            },
            fail: function(t) {
